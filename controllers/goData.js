        ...
        switch (status) {
            ...
            case nssmValidStatuses.ServiceRunning:
                ...
                compareServiceAPIWithAppAPI((err, identical) => {
                    ...
                    detectAppStartFromLog({}, events, callback);
                    restartGoDataAPIService(() => {
...
/**
 * Stop Go.Data API as a service using nssm.exe
 * @param callback
 */
function stopGoDataAPIService(callback) {
    logger.info('Stopping Go.Data API Service...');
    let command = ['stop', goDataAPIServiceName];
    runNssmShell(command, {requiresElevation: true}, callback);
}
 ...
 * @param callback (err, boolean) => {} - True if node service is running
 */
function isGoDataServiceRunning(callback) {
...
module.exports = {
    ...
    killGoData,
    setAppPermissions,
    stopGoDataAPIService,
    isGoDataServiceRunning,
    startGoDataAsService
};
